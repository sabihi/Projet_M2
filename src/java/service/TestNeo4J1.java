...
public class TestNeo4J1 {
    ...
    private final static double seuil=0.5;
    
    public GraphDatabaseService getGhraphBD(){
...
//        GraphDatabaseFactory generator = new GraphDatabaseFactory();
//        graphDb = generator.newEmbeddedDatabase(DB_PATH);
//        TestNeo4J1 testNeo4J = new TestNeo4J1();
...
//    }
    ...
    public void createServicesGraph() throws SQLException {  //WHERE id>=6000 and id<=6600 //WHERE id>=6583 and id<=6599
        ResultSet resultSet = ConnectMSSQLServer.select("SELECT * FROM [dbo].[ws] WHERE categoryid='Recommendations'");
    ...
    public void createUsersGraph() throws SQLException {//WHERE id>=2250 and id<=2260 //WHERE id>=2000 and id<=3000
        ResultSet resultSet = ConnectMSSQLServer.select("SELECT * FROM [dbo].[userinfo] WHERE id>=2000 and id<=2025");
    ...
    public void setWebServiceRelations() {
        ...
        for (int i = 0; i < services.size(); i++) {
            ...
            for (int j = 0; j < services.size(); j++) {
                if (i != j) {
                    List<String> otherService = services.get(j);
                    similarity_Two_Rows.add(jaccard.jaccard_coeffecient(myService, otherService));
                } else {
        ...
        for (int i = 0; i < servicesSimilarity.size(); i++) {
            ...
            for (int j = i + 1; j < servicesSimilarity.get(0).size(); j++) {
                if (servicesSimilarity.get(i).get(j) > seuil && servicesSimilarity.get(i).get(j) != 1) {
                    ...
                    //webService1.add_similare_ws(webService2);
    ...
    public void clustering(List<List<Double>> servicesSimilarity) {
        for (int i = 0; i < servicesSimilarity.size(); i++) {
            ...
            nodes.add(getNodeById(i));
            for (int j = i + 1; j < servicesSimilarity.get(0).size(); j++) {
                if (servicesSimilarity.get(i).get(j) > seuil && servicesSimilarity.get(i).get(j) != 1) {
                    idservices.add(j+1);
                    nodes.add(getNodeById(j+1));
                    String s[] = ((String) getNodeById(j+1).getProperty("tags")).split(",");
                    int taille = s.length;
                    for (String string : s) {
                        set1.add(string);
                    }
            ...
            for (int k = 0; k < idservices.size(); k++) {
                for (int x = 0; x < servicesSimilarity.size(); x++) {
                    y=idservices.get(k);
                    if (servicesSimilarity.get(y).get(x) > seuil && servicesSimilarity.get(y).get(x) != 1) {
            ...
            if (nodes.size() > 1) {
                ...
                node_C.setProperty("name", set1.iterator().next());
